...
对齐 PRD v6.2 与实现契约 v2.7，采用 **7 模块** 结构（监控/回路/评估/诊断/整定/配置/系统），引入双轴导航（实体轴回路工作台 + 职能轴评估/诊断/整定）与工程/操作分离原则。各业务模块仍遵循"配置→运行→分析"三态自包含原则：

| 模块 | 配置态 | 运行态 | 分析态 |
|---|---|---|---|
| 监控 | — | 系统概览、回路实时、运行驾驶舱 | — |
| 回路 | — | 回路工作台（单页四区）、回路实时监控 | — |
