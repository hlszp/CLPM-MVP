...
日期：2026-08-09（对齐 PRD v6.2、UI/UX v6.3 与重构后实现契约 v2.9）
状态：active-baseline
...
| 重构后实现契约 | `docs/设计文档/00-BASELINE/implementation-contract.md` | v2.9 |
...
对齐 PRD v6.2 与实现契约 v2.9，采用 **6 个一级模块** 结构（监控/评估/诊断/整定/配置/系统）。回路作为核心实体，其运行态统一进入监控下的回路工作台，结构性配置进入配置模块；预警事件进入监控，预警规则进入配置。保留双轴导航（监控下实体轴回路工作台 + 评估/诊断/整定职能轴）与工程/操作分离原则。各业务模块仍遵循"配置→运行→分析"三态自包含原则：
