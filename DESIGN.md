...
日期：2026-08-10（对齐 PRD v6.2、UI/UX v6.3 与重构后实现契约 v2.10）
状态：active-baseline
...
| 重构后实现契约 | `docs/设计文档/00-BASELINE/implementation-contract.md` | v2.10 |
...
当本文件与实现契约冲突时，以 `docs/设计文档/00-BASELINE/implementation-contract.md` 为准；当业务范围与安全边界冲突时，以 PRD v6.1 为准。本文件保留横切设计约束。
...
对齐 PRD v6.2 与实现契约 v2.10，采用 **6 个一级模块** 结构（监控/评估/诊断/整定/配置/系统）。回路作为核心实体，其运行态统一进入监控下的回路工作台，结构性配置进入配置模块；预警事件进入监控，预警规则进入配置。保留双轴导航（监控下实体轴回路工作台 + 评估/诊断/整定职能轴）与工程/操作分离原则。各业务模块仍遵循"配置→运行→分析"三态自包含原则：

| 模块 | 配置态 | 运行态 | 分析态 |
|---|---|---|---|
| 监控 | — | 系统概览、回路实时、运行驾驶舱 | — |
| 回路（监控下实体入口） | — | 回路工作台（单页四区：监控/评估/诊断/整定） | — |
| 评估 | 指标配置、引擎规则配置（迁入 `/config/metric`） | KPI 看板、低效排行、评估任务 | 性能统计、KPI 报表 |
| 诊断 | 诊断配置（迁入 `/config/diagnosis`） | 诊断任务、详情三区、异常跟踪 | 诊断统计、回路分析 |
| 整定 | — | 整定工作台、整定详情（单页 4 锚点） | 整定效果统计、知识库 |
| 配置 | 链路/测点/回路/数据源/指标/诊断/预警规则结构性配置 | — | — |
| 系统 | 用户/角色/审计/自动报表/LLM 配置 | — | — |
...
`docs/设计文档/00-BASELINE/implementation-contract.md` 与 `frontend/apps/web-antd/src/router/routes/modules/` 是当前菜单结构与路由结构的事实来源。原型 `menuConfig.ts` 仅作为历史原型入口，不再单独派生第二份真相。
