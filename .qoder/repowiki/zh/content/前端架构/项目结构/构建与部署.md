...
## 更新摘要
**所做更改**
- 更新了开发服务器端口配置，从硬编码值(7101/7100)同步到MVP隔离端口方案(后端17101，前端15666)
- 增强了Makefile中的端口约定说明和dev-all目标注释
- 更新了前端代理配置以使用新的MVP后端端口
- 添加了MVP端口隔离方案的详细说明
- 修正了故障排查指南中的端口相关信息
...
**重要更新**：本项目已采用 MVP 端口隔离方案，所有服务端口在原项目基础上加 10000，确保与原 CLPM 项目在同一台机器上并行运行而互不干扰。
...
```mermaid
graph TB
A["Monorepo 根<br/>pnpm + turbo"] --> B["apps/web-antd<br/>Vite 应用 (端口: 15666)"]
...
- 开发代理：Vite dev server 将 /api 请求代理到后端 127.0.0.1:17101（MVP 隔离端口），并开启 WebSocket 支持。
...
- 开发代理：/api 转发至 http://127.0.0.1:17101（MVP 隔离端口），并启用 ws 以支持 WebSocket。
...
BuildMode --> |development| DevServer["启动开发服务器<br/>端口: 15666 (MVP隔离)<br/>热更新/代理/api -> 17101"]
...
### 开发环境配置
- **MVP 端口隔离**：前端开发服务器默认端口为 15666（原项目为 5666），后端 API 端口为 17101（原项目为 7101）。
...
**重要更新**：为确保 MVP 项目与原 CLPM 项目在同一台机器上并行运行而互不干扰，采用了完整的端口隔离方案。

#### 端口映射表
| 服务 | 原项目端口 | MVP 端口 | 说明 |
|------|------------|----------|------|
| 前端开发服务器 | 5666 | **15666** | Vite dev server |
| 后端 API 服务器 | 7101 | **17101** | FastAPI + Uvicorn |
| PostgreSQL | 7102 | **17102** | 关系型数据库 |
| Redis | 7103 | **17103** | 缓存与消息队列 |
| TDengine | 7104/7115 | **17104/17115** | 时序数据库 |

#### Makefile 开发命令
- `make dev`：启动后端开发服务器（端口 17101，MVP 隔离端口）
- `make dev-frontend`：启动前端开发服务器（端口 15666，MVP 隔离端口）
- `make dev-all`：启动所有开发服务（基础设施 + 后端 + 前端）

#### Docker 隔离
- 容器名前缀：`clpm-mvp-*`（原项目为 `clpm-*`）
- 数据卷前缀：`clpm_mvp_*`（原项目为 `clpm_*`）
- 网络名：`clpm-mvp-dev-net`（原项目为 `clpm-dev-net`）
- Compose 项目名：`clpm-mvp-dev`（原项目为 `clpm-dev`）

**章节来源**
- [Makefile:30-42](file://Makefile#L30-L42)
- [deploy/docker/docker-compose.dev.yml:7-11](file://deploy/docker/docker-compose.dev.yml#L7-L11)
...
**重要更新**：由于采用了 MVP 端口隔离方案，请特别注意以下端口配置：

- **前端访问地址**：http://localhost:15666（而非原来的 5666）
- **后端 API 地址**：http://localhost:17101（而非原来的 7101）
- **数据库连接**：PostgreSQL 17102, Redis 17103, TDengine 17104/17115
...
- **代理异常**
  - 确认后端服务监听 127.0.0.1:17101（MVP 端口），且未被其他进程占用。
  - 检查防火墙与 IPv4/IPv6 绑定，确保代理可达。
  - 注意：使用 127.0.0.1 而非 localhost，避免 IPv6 解析问题。

- **端口冲突**
  - 检查是否有原 CLPM 项目或其他服务占用了 15666/17101 端口。
  - 使用 `lsof -i :15666` 和 `lsof -i :17101` 检查端口占用情况。
...
- **测试失败**
  ...
  - E2E：确保浏览器驱动安装与测试环境可用，注意 E2E_BASE_URL 应指向 15666 端口。

**章节来源**
- [frontend/vitest.config.ts:5-28](file://frontend/vitest.config.ts#L5-L28)
- [frontend/apps/web-antd/vite.config.ts:6-20](file://frontend/apps/web-antd/vite.config.ts#L6-L20)
- [frontend/turbo.json:1-13](file://frontend/turbo.json#L1-L13)
- [backend/app/core/config.py:140-146](file://backend/app/core/config.py#L140-L146)

## 结论
本项目通过 Turborepo 统一编排、@vben/vite-config 集中管理构建能力，实现了开发高效、生产优化的前端工程化方案。**特别重要的是，MVP 端口隔离方案确保了与原项目的完全解耦**，使团队可以在同一环境中并行开发和测试。借助 PWA、压缩、分包与 CDN 缓存策略，兼顾了首屏性能与长期缓存收益。CI/CD 与本地脚本打通了从代码到上线的完整链路，便于持续交付与回滚。
...
- **解决常见问题**
  - **代理 502**：确认后端地址为 127.0.0.1:17101（MVP 端口），优先使用 127.0.0.1 而非 localhost。
  - **端口冲突**：检查 15666/17101 端口是否被占用，必要时调整端口配置。
