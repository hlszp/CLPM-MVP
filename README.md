...
| 测试 | pytest（4812 collected，2026-08-26）+ vitest（434 passed）+ Playwright E2E（94 例：75 passed 基线 + 13 环境类既有失败，见 06-UIUX 出口报告） |
...
| 首版范围 | 工作台 v2.0（5 Tab 驾驶舱）、监控（装置总览/回路监视/预警事件/关注队列/回路工作台）、回路结构配置、性能评估（看板/指标分析/排行/任务/统计/适用性阈值）、诊断中心（诊断/异常跟踪/统计）、回路整定（门禁 L3+）、处置 v2.0（建议审核+工单执行）、统计报告（管理总览+5 类报告+订阅配置）、预警规则归配置、系统管理（基础信息/用户/审计/权限矩阵/字典/模块管理） |
