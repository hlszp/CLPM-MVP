...
| 当前版本 | 产品文档基线 **v6.3**；实现契约 v2.9；后端运行时默认 `1.0.0`；发布版本以 Git tag 为准。上一轮基线（2026-08-09）：pytest 4139 passed / vitest 466 全绿 / E2E 75 passed（13 环境类既有失败归因见出口报告）/ `alembic check` 退出码 0 |
...
| 模块架构 | 6 个一级模块（契约 v2.9）：监控/评估/诊断/整定/配置/系统；回路运行态归入监控下回路工作台，预警事件归监控、规则归配置；双轴导航和各模块"配置→运行→分析"三态自包含 |
...
| 工程主约束 | PRD v6.2 负责产品需求；实现契约 v2.9 负责当前 IA/路由/API/权限/状态机/KPI；UI/UX v6.3 负责视觉与交互（继承 v6.2 色彩约定表 v1.0 + 文案词表 v1.0） |
