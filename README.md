...
CLPM-MVP 是面向危化企业控制回路的绩效治理与优化闭环平台，覆盖"**监控 → 评估 → 诊断 → 整定 → 处置 → 统计报告**"完整业务闭环 + 管理决策视图（2026-08-23 IA 优化 P0~P4 完成），提供：

- **监控**：关注队列（ALERT/DEGRADATION/DATA_QUALITY 三来源聚合 + 四级优先级 + 角色化动作）/ 回路工作台单页多区（运行态 + 评分趋势 + 诊断卡 + 活跃关注项 + L2 适用性横幅）/ 回路台账（新增适用性列+筛选）/ Tag 关联 / 实时监控；装置总览展示 L0~L4 适用性 5 段堆叠横条
- **性能评估**：KPI 看板 / 节点排名 / 统计分析 / 指标配置（指标定义 / 引擎规则 / 类型权重 / 级别权重 / 异常值检测参数 / KPI 算法参数 / **适用性阈值 Tab**）/ 可信度标识；L0/L1 回路灰色"不适用"分类独立不计入"差"
...
- **统计报告**（一级菜单 2026-08-23 IA 优化 P0 落地）：管理总览（固定 12 格骨架 + 成熟度 S1/S2/S3 自适应 + S2/S3 阶段内容）/ 绩效报告（由评估-KPI 报表迁入）/ 诊断报告（基于 DiagnosisRun 重建）/ 处置报告（由处置-统计迁入）/ 收益报告（整定前后 KPI 对比 + 自控率提升曲线 + 装置标杆）/ 订阅配置（由系统-自动报表迁入，原"报告订阅"改名）；旧路径全局 redirect 就位
- **智能预警**：规则引擎（阈值/漂移/组合/可信度 DSL）+ 事件流（确认→处置→归档，误报标记/撤销）+ 顶栏通知铃铛 /ws/alerts 实时推送；预警→诊断联动受热插拔开关控制自动降级
- **系统管理**：用户管理 / 审计日志 / 权限矩阵（未启用模块列虚线灰显）/ 自动报表（→ 已迁统计报告→订阅配置）/ 字典管理（测点类型/参数类型/回路类型三类可配置字典）/ **模块管理**（模块热插拔开关，诊断/整定/处置按客户阶段弹性启用禁用，重启生效，含依赖校验和禁用确认弹窗）
...
| 当前版本 | 产品文档基线 **v6.2**；实现契约 v2.11；后端运行时默认 `1.0.0`；发布版本以 Git tag 为准。本轮基线（2026-08-23）：**IA 优化 P0~P4 全部落地**（统计报告升一级菜单 + 成熟度 S1/S2/S3 自适应管理总览、模块热插拔诊断/整定/处置弹性交付、适用性评估 L0~L4 分层+诊断整定门禁、2 项验收 bug 修复、27 回路真实数据一致率 ≥85%）；左导航顺序 8 菜单 order=监控1/评估2/诊断3/整定4/处置5/统计报告6/配置7/系统8；上轮（2026-08-14）：页面标杆设计落地（系统概览页 R1~R4 完整重写：R1 页头 segmented 时间窗 + R2 全厂锚点 KPI 六指标 + R3 工厂模型树联动 + R4 帕累托对比 + 微型扇形图；关注队列页列宽优化）+ dashboard API AlertEvent 导入修复 + 时间窗北京时间对齐；上上轮（2026-08-11）：IA 评审 Backlog P2-22/P3-33 闭环（风险统计 + 异步 PDF 导出）+ Celery Worker 清理加固 + KPI 指标条紧凑化；pytest 4241 passed / vitest 434 passed / ruff✅ / check:type✅ |
| 首版主线 | Phase 1 (MVP/V1.0)：跑通"自动评估、自动诊断、轻量跟踪、模块弹性交付、适用性分层"闭环 + 管理决策视图 |
| 首版范围 | 监控（系统概览/跨回路扫视/回路工作台/预警事件）、回路结构配置、性能评估（看板/排行/任务/统计/适用性阈值）、诊断中心（诊断/异常跟踪/统计）、回路整定（门禁 L3+）、处置 v2.0（建议审核+工单执行）、统计报告（管理总览+5 类报告+订阅配置）、预警规则归配置、系统管理（用户/审计/权限矩阵/字典/模块管理） |
| 模块架构 | **8 个一级模块**：监控/评估/诊断/整定/处置/统计报告/配置/系统（2026-08-23 调整，原 6 模块基础上新增统计报告一级化，处置统计从处置迁出、KPI 报表从评估迁出、自动报表从系统迁出统一归口；统计报告成熟度 S1/S2/S3 自适应，管理员可锁定）；诊断/整定/处置支持模块热插拔，按客户阶段弹性启用；回路运行态归入监控下回路工作台，预警事件归监控、规则归配置；双轴导航和各模块"配置→运行→分析"三态自包含 |
