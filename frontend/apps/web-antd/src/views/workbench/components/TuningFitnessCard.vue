...
const helpItems = [
  { label: '环形图', text: 'L0~L4 五档分级占比（按 KpiSnapshotHourly.score 分桶）。' },
...
const C = 2 * Math.PI * 42; // ≈ 263.89 周长
...
type Seg = { color: string; count: number; key: string; len: number; offset: number; pct: number };
const segments = computed<Seg[]>(() => {
  ...
  return LEVEL_ORDER.map((k) => {
    ...
    const len = (count / t) * C;
    const seg: Seg = { color: LEVEL_COLORS[k] ?? '#1F4E79', len, offset: acc, key: k, count, pct };
    acc += len;
    return seg;
  });
});
      ...
      <div class="flex h-full w-full items-center gap-[6px]">
        ...
        <div class="relative flex h-full max-h-[150px] w-[150px] flex-none items-center justify-center">
          ...
          <div class="absolute inset-0 flex flex-col items-center justify-center">
            ...
            >{{ level }}</span>
            <span class="mt-[2px] text-[9.5px] font-medium text-[#595959]">{{ levelLabel }}</span>
            <span class="mt-[2px] text-[10px] font-bold tabular-nums" :style="{ color: levelColor }">{{ score }}<span class="text-[7.5px] text-[#8C8C8C]">/100</span></span>
          </div>
        </div>
        <!-- 图例 + 数字刻度 -->
