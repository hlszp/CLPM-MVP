...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "plant_node_id", "stat_date", name="uk_kpi_node_snapshot_daily_node_date"
        ),
    )

    op.create_index(
        "idx_kpi_node_snapshot_daily_node_id", "kpi_node_snapshot_daily", ["plant_node_id"]
    )
    op.create_index(
        "idx_kpi_node_snapshot_daily_stat_date", "kpi_node_snapshot_daily", ["stat_date"]
    )
    op.create_index("idx_kpi_node_snapshot_daily_status", "kpi_node_snapshot_daily", ["status"])
    op.create_index(
        "idx_kpi_node_snapshot_daily_node_date",
        "kpi_node_snapshot_daily",
    ...
    op.execute("COMMENT ON COLUMN kpi_node_snapshot_daily.stat_date IS '统计日期（DATE）'")
    op.execute(
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "plant_node_id", "stat_month", name="uk_kpi_node_snapshot_monthly_node_month"
        ),
    )

    op.create_index(
        "idx_kpi_node_snapshot_monthly_node_id", "kpi_node_snapshot_monthly", ["plant_node_id"]
    )
    op.create_index(
        "idx_kpi_node_snapshot_monthly_stat_month", "kpi_node_snapshot_monthly", ["stat_month"]
    )
    op.create_index("idx_kpi_node_snapshot_monthly_status", "kpi_node_snapshot_monthly", ["status"])
    op.create_index(
        "idx_kpi_node_snapshot_monthly_node_month",
        "kpi_node_snapshot_monthly",
    ...
    op.execute("COMMENT ON COLUMN kpi_node_snapshot_monthly.stat_month IS '统计月份（DATE，月初）'")
    op.execute(
