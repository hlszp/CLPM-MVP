...
2. 新增 5 个对齐 CALCULATOR_REGISTRY 的 metric_code：
   （fast_rate/stability_rate/stiction_index/output_trip_index/settling_time）
3. effective_auto_rate 的 depends_on：["auto_mode_rate","saturation_rate"] → NULL
4. 新增范围查询索引 idx_kpi_snapshot_ts_loop (ts_start, loop_id)

注：oscillation_rate 保持 BASE / ["pv","sp"] / pv_valid && sp_valid（与 ADS §指标数据
需求契约及 OscillationRateCalculator._get_masked_pair("pv","sp") 一致；算法基于
控制偏差 E=PV-SP，需要 SP）。

设计依据：评估算法优化改进方案 v1.0 Phase 1 + ADS 指标数据需求契约
...
def upgrade() -> None:
    ...
    # ========================================================
    # 3. oscillation_rate 保持 BASE / ["pv","sp"] / pv_valid && sp_valid 不变
    #    （与设计文档 ADS §指标数据需求契约及 OscillationRateCalculator 一致；
    #     算法基于控制偏差 E=PV-SP，_get_masked_pair("pv","sp") 需要 SP 信号。
    #     此前误改为 PVOP_HF/["pv","op"] 导致 bundle 无 SP → INCONCLUSIVE → 全 NULL）
    # ========================================================
...
def downgrade() -> None:
    ...
    # oscillation_rate 保持不变（upgrade 未修改），无需恢复
