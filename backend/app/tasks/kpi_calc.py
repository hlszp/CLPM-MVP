    ...
    try:
        ...
        if task_id:
            try:
                ...
                logger.warning("更新 FAILED 状态失败: task_id=%s", task_id, exc_info=True)
        raise


# ---------------------------------------------------------------------------
# 历史数据导入任务（import_history_data）— 从远端 API 拉取并写入本地 TDengine
# 实际逻辑由 app.services.data_import.import_history_data 实现，
# 此处为 Celery 任务包装器，供 /loops/data-import/start 接口 .delay() 调用。
# ---------------------------------------------------------------------------
...
def import_history_data(
    ...
    """从远端 HTTP API 导入历史数据到本地 TDengine（Celery 任务包装器）。
    ...
    from app.services.data_import import (
        import_history_data as _do_import,
    )

    logger.info(
        "历史数据导入 Celery 任务启动, celery_id=%s, task_id=%s, loops=%d, range=%s~%s",
        self.request.id,
        task_id or "(none)",
        len(loop_ids),
        ts_start,
