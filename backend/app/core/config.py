...
class Settings(BaseSettings):
    ...
    # 2026-09-20：60→30——链路探针实证 CGNAT 间歇杀连接（zpdev 出口 1.5h 内
    # 4 次，检测窗口每多 1 秒=多丢 1 秒数据）。分片后每片 ~2200 位号、常态
    # ~30 msg/s，30s 全静默必为死连接，无误判风险
    SIGNALR_STALL_TIMEOUT_SECONDS: int = 30  # 默认 30 秒
