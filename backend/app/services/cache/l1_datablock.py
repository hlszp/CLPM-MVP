...
def _data_block_from_dict(data: dict[str, Any]) -> DataBlock:
    ...
    旧缓存兼容：``loop_confidence_level`` / ``loop_valid_rate`` 字段
    在可信度统一 Phase 2 之后才加入序列化。旧缓存命中时这两个字段缺失，
    需从 validity 重新计算 loop_valid_rate 并重评估可信度等级，
    ...
    if loop_confidence_level is None or loop_valid_rate is None:
        ...
        logger.info(
            "L1 cache 旧缓存兼容：重算回路级可信度: loop_valid_rate=%.4f, level=%s",
            loop_valid_rate,
