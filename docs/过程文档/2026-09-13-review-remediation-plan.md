...
| G18~G27 | S3 | 进行中（第 6 轮） | `2e3ea3b3` | G20/G21/G23主路径/G25 已修复；G24 确认待裁决；G18/G19/G22 **撤销（误诊）** | 余 G26/G27 待做。G18「尖峰抬高 A」与 G19「正圆不检出」经事实来源比对判定为规格自身要求，已固化为一致性/契约用例；G19 连带修掉 stiction 诊断侧量程口径分叉；G21 为真实缺陷（SOPDT 按 fail-closed 拒绝，规格 6.3~6.7 仅定义 FOPDT 整定公式）；G22 连带修掉级 2 判定门（OR 改 AND，原实现使 0.3+0.3 融合 0.1552 也判 VALVE 并架空级 7 兜底）；G23 子claim 4 属实——可信度统一方案 Phase 1（2026-08-04）统一的口径被 R14-2（2026-09-06）只落在一处的时间覆盖因子重新劈开，而 Phase 1 守卫是结构性的故保持全绿，已修 KPI 链主路径，诊断/整定两链待续；G24 确认 mode 为死参数（手动段可整段进辨识），修法需规格裁决（是否只取 AUTO 段 / 或承认不区分模式）；G25 属实——ARX is_stable 实测 4 例错 3 例（一阶把极点 -a1 误当成 a1，两方向都错），已改按极点模；S3 命中率 4 修 + 1 待裁决 + 3 误诊，误诊模式为"把规格自身规定的行为读成实现缺陷"。详见收口报告 17~21 |
| G28~G32 | S4 | 未开始 | — | — | — |
| G33~G38 | S5 | 未开始 | — | — | — |
| G39~G50 | S1/S6 | 未开始 | — | — | — |
