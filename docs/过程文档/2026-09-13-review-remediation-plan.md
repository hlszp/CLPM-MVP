...
| G18~G27 | S3 | 进行中（第 4 轮） | `25eb93c7` | G20 已修复；G21 已修复；G18/G19 **撤销（误诊）** | 余 G22~G27 待做。G18 的「尖峰抬高 A」与 G19 的「正圆不检出」均经事实来源比对判定为规格自身要求，已固化为一致性/契约用例；G19 复核连带修掉一处真实量纲缺陷（诊断侧 stiction 量程口径与 KPI 侧分叉）；G21 为真实缺陷，按 fail-closed 拒绝 SOPDT（规格 6.3~6.7 仅定义 FOPDT 整定公式）；G22 两项原判均不成立，但连带修掉级 2 判定门（confidence>=0.7 与 ≥2 命中由 OR 改 AND，原实现使 0.3+0.3 融合 0.1552 也判 VALVE 并架空级 7 兜底）。S3 命中率 2/5，三项误诊共同模式为"把规格自身规定的行为读成实现缺陷"，故 G23~G27 一律先做事实来源比对。详见收口报告 17/18/19 |
| G28~G32 | S4 | 未开始 | — | — | — |
| G33~G38 | S5 | 未开始 | — | — | — |
| G39~G50 | S1/S6 | 未开始 | — | — | — |
