...
| G18~G27 | S3 | 进行中（第 5 轮） | `19adacd1` | G20/G21 已修复；G18/G19 **撤销（误诊）** | 余 G23~G27 待做。G18「尖峰抬高 A」与 G19「正圆不检出」经事实来源比对判定为规格自身要求，已固化为一致性/契约用例；G19 复核连带修掉 stiction 诊断侧量程口径分叉；G21 为真实缺陷（SOPDT 按 fail-closed 拒绝，规格 6.3~6.7 仅定义 FOPDT 整定公式）；G22 两项原判均不成立，连带修掉级 2 判定门（OR 改 AND，原实现使 0.3+0.3 融合 0.1552 也判 VALVE 并架空级 7 兜底）；G23 子claim 4 属实——可信度统一方案 Phase 1（2026-08-04）统一的口径被 R14-2（2026-09-06）只落在一处的时间覆盖因子重新劈开，而 Phase 1 守卫是结构性的故保持全绿；已修 KPI 链主路径（门禁+落库），诊断/整定两链待续，其余子claim 1/2 不成立或部分成立、3 待验。S3 命中率 3/6，误诊共同模式为"把规格自身规定的行为读成实现缺陷"，故 G24~G27 一律先做事实来源比对。详见收口报告 17~20 |
| G28~G32 | S4 | 未开始 | — | — | — |
| G33~G38 | S5 | 未开始 | — | — | — |
| G39~G50 | S1/S6 | 未开始 | — | — | — |
