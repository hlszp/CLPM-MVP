...
| G33~G38 | S5 | **部分完成**（第 19~20 轮核实） | 见 S5 提交 | G33 裸 HTTPException 已达标且已补 CI 断言；G38 已落地；**G37 状态机无锁已确认但本轮未修**（约 8 个迁移站点为"读守卫+无条件写"，handling.py 全文无 with_for_update；修法需加锁读变体+真 PG 并发测试，预算内无法完成完整证据链，按"未验证的修复不上线"处置） | 仍待办：G33 错误码集中定义（182 个，重构级）、G34 归属校验、G35 分页/时间契约、G36 审计缺口；G37 移交材料见阶段验收记录 §13。详见 §12/§13 |
| G39~G50 | S1/S6 | 未开始 | — | — | — |
