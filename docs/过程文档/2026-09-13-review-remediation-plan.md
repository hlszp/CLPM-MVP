...
| **G38 运维端点未鉴权** | **S5** | **已落地** | 见 S5 提交 | 2 项回归：/health 无 token 仍 200（探针依赖）、/health/db-connections 无 token 到 401 且不回显 byApp；503 分支异常原文已脱敏 | nginx location /health 仍为前缀匹配（未改 nginx，由后端鉴权兜住）；/health/ready 失败信息仍回显异常类别名，未脱敏 |
| **G33 错误处理旁路** | **S5** | **部分落地** | 见 S5 提交 | DSL ValidationError 全局注册为 400 ERR_ALERT_DSL_INVALID（新增调用点自动覆盖）；reports.py 五处裸 HTTPException 换 BizError 并赋稳定领域码 | core/modules.py:241 本轮已一并处理；**错误码集中定义（app/core/error_codes.py）未做**，182 个字面量错误码仍分散 |
...
| G18~G27 | S3 | **清单复核完毕**（第 12 轮） | `7232e4be` | G20/G21/G23主路径/G25/G26/G27#2b/G27#5a/G27#3可观测化 已修；G24、G27#1 待办；G27#5b、G27#3阈值 待裁决；G18/G19/G22/G27#4 **误诊** | G27 七项全部复核：3 修 + 1 确认未修 + 2 待裁决 + 1 误诊 + 1 非缺陷。关键结论：#2b log(abs(p)) 伪装负实极点属实已修、ZOH 重复计拍不成立；#5a Ljung-Box 自由度未扣已估参数属实已修、#5b R2 免检待裁决；#3 IV 病态但非奇异时 solve 静默成功，已改为记录 cond/solve_mode；#1 延误 BIC 跨样本量比较成立但修法未验证（x 已 xfail 固化）；#4 误诊；#7 属预留结构。S3 累计 8 修 + 2 待办 + 2 待裁决 + 4 误诊。详见收口报告 17~27 |
| G28~G32 | S4 | **已核实完成**（第 18 轮复核） | 见 S4 提交 | 7 个专项测试文件 40 passed / 0 failed；G28 用例构造**真实 Scheduler** 断言条目从运行中的 scheduler.schedule 消失（并用 conf 未改动反向证明改的是活调度表），G29 失败熔断 5 条、G30 锁不变量 2 条、G32 指标与 visibility_timeout 5 条 | 本行原记"未开始"系**状态陈述过期**（S4 早已修复且有专测），本轮经实跑核实后更正；详见阶段验收记录 §11 |
| G33~G38 | S5 | **部分完成**（第 19 轮核实） | 见 S5 提交 | G33 裸 HTTPException 已达标（全仓 0 处 / BizError 437 处）且本轮**补建 CI 断言**（test_no_bare_http_exception_g33，6 passed，含"守卫会触发"的自证用例）；G38 已落地（test_health_auth_g38） | **本行原记"未开始"系状态陈述过期**，且与同表 G38 行「已落地」自相矛盾，本轮更正。仍待办：G33 错误码集中定义（182 个，重构级）、G34 归属校验、G35 分页/时间契约、G36 审计缺口、G37 状态机无锁 —— 均未复核。详见阶段验收记录 §12 |
| G39~G50 | S1/S6 | 未开始 | — | — | — |
