...
| G18~G27 | S3 | 进行中（第 7 轮） | `36ae7463` | G20/G21/G23主路径/G25/G26 已修复；G24 确认待裁决；G18/G19/G22 **撤销（误诊）** | 余 G27 待做。G18「尖峰抬高 A」与 G19「正圆不检出」经事实来源比对判定为规格自身要求；G19 连带修掉 stiction 诊断侧量程口径分叉；G21 为真实缺陷（SOPDT fail-closed）；G22 连带修掉级 2 判定门（OR 改 AND）；G23 子claim 4 属实（R14-2 只落一处的时间覆盖因子劈开口径，Phase 1 守卫是结构性的故全绿），已修 KPI 链主路径；G24 确认 mode 为死参数、修法需规格裁决；G25 属实（ARX is_stable 实测 4 例错 3 例），已改按极点模；G26 前两项误诊（门禁三条与级 0 逐条一致），第三项成立——32~99 点窗口把「数据不足」渲染成「未检出」，已改为 executed=False + skip_reason。S3 命中率 5 修 + 1 待裁决 + 3 误诊。详见收口报告 17~22 |
| G28~G32 | S4 | 未开始 | — | — | — |
| G33~G38 | S5 | 未开始 | — | — | — |
| G39~G50 | S1/S6 | 未开始 | — | — | — |
