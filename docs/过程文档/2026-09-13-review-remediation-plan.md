...
| G18~G27 | S3 | 进行中（第 8 轮） | `f466c7dd` | G20/G21/G23主路径/G25/G26 已修复；G24 待裁决；G18/G19/G22/G27#4 **误诊** | G27 为"待验证清单"（原判自述未逐行复核）：#4 PE 激励门禁"恒真"经噪声扫描实测**不成立**（条件数分支在 eps<=1e-4 共线度上均触发），阈值标定（1e4/1e6 对应 |rho|=1-2e-8/1e-12）登记为残余观察待规格裁决；#6 与 G24 同一问题；#7 unitConversion 属 V62-P3-007 预留结构（已在 §18 见代码），非隐性缺陷；#1/#2/#3/#5 尚未复核。S3 累计 5 修 + 1 待裁决 + 4 误诊，误诊模式为"把规格自身规定的行为读成实现缺陷"。详见收口报告 17~23 |
| G28~G32 | S4 | 未开始 | — | — | — |
| G33~G38 | S5 | 未开始 | — | — | — |
| G39~G50 | S1/S6 | 未开始 | — | — | — |
