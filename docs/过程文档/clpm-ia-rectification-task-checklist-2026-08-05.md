...
| 编号 | 任务名称 | 描述 | 模块 | 负责人 | 预计完成 | 状态 | 依赖 | 验收标准 |
|---|---|---|---|---|---|---|---|---|
| P2-01 | 结构化诊断报告 | 诊断结果卡片重构：原因排序+概率+建议下一步+预估改善效果；不再只说"有振荡"，给出"PID 过激 65% / 阀门粘滞 25% / 外扰 10%" | 诊断中心 | 待分配 | 2026-09-15 | 📦 | P1a 合并 | 诊断卡片含 3 段：原因排序/建议下一步/预估改善。已覆盖：ClpmStructuredDiagnosisReport 组件（原因排序概率条+折叠面板含根因/建议/预估改善/紧急度/动作类型），8 标签结构化报告数据（DIAGNOSIS_STRUCTURED_REPORT），集成到诊断详情页替换原始证据展示 |
