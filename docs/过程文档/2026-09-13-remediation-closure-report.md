...
## 25. G27 复核（第 3 项）：延迟搜索的 BIC 跨样本量比较——**成立，已确认但尚未修复**

### 25.1 事实依据

BIC 的似然项 `n·ln(σ²)` 随 n **线性**缩放，故只有 n 相同才可比。
`_search_delay`（`pipeline.py`）对每个候选 d 各用 `res.n_samples`，而 d 越大
回归行越少（n 随 d 递减），因此**跨候选的 BIC 不可比**。

### 25.2 实测（真值 d=8，1200 点）

| d | n | σ² | BIC |
|---|---|---|---|
| 0 | 1199 | 6.312e-03 | −6059.13 |
| 4 | 1195 | 6.333e-03 | −6034.90 |
| 8 | **1191** | **1.896e-04** | **−10193.27** |
| 12 | 1187 | 6.353e-03 | −5990.72 |

"错误候选"的 BIC 随 d **单调变差**（−6059 → −5991），该 artifact 贡献约
**+68 BIC / 12 步 ≈ 每单位 d 5.7**，与合法复杂度罚项 `ln(n)≈7.1` **量级相当**——
即延误搜索对大延迟的保守度约翻了一倍；余量小时会误选偏小的 d。
本样例因 d=8 的拟合改善极大（σ² 降 33 倍）仍被正确选中，
但这是"改善足够大以至于掩盖了偏差"，不是偏差不存在。

### 25.3 未修复的原因（一次失败的修复尝试，已回退）

我实现了"公共窗重算残差"的修法：取 `t0 = max(na, d_max+nb)`，用各候选的
拟合系数在该窗上重算残差方差，并以同一 `n_common` 计罚。**验证未通过**：
真值 d=8 反而落败（d=7 以 −6060 胜出，d=8 仅 −5211）。定位到根因是
`u(t-d-j)` 的索引与 `identify_arx` 的回归约定**差一拍**。

**该实现已 `git checkout` 回退**。理由：延误选择直接决定 FOPDT/SOPDT 的
`theta`，而 IMC/ZN/Cohen-Coon 公式对 θ 高度敏感——在延误选择上留一个
**未经验证**的改动，比保留一个已知有偏的原实现更危险。

### 25.4 证据链（可执行）

| 环节 | 证据 |
|---|---|
| 缺陷固化 | test_delay_bic_g27.py：`xfail(strict=True)` 用例把 artifact 放大到可判定（极小残差白噪声 PV 使 σ² 各候选近似恒定，BIC 差异应仅反映噪声，实测却随 d 出现大幅单调漂移）；修复后该用例按设计翻转为 XPASS 并强制删除标记 |
| 回归护栏 | 同文件另一用例断言"具明确滞后结构的数据仍应被正确识别"（当前实现可做到，**通过**） |
| 集成证据 | 全量门禁通过（本轮无源码改动） |
| 残余风险 | ① 缺陷仍在生产路径上（延误可能偏小）；② 正确实现需先读准 `identify_arx` 的回归索引约定（本次未及），建议下轮先写一个"给定系数、手算残差"的对照用例再改；③ 修复会改变延误选择结果，属行为变更，需与 G24 一并纳入回归观察 |

### 25.5 G27 状态更新

| 项 | 状态 |
|---|---|
| 1 BIC 跨样本量比较 | **已复核 → 成立，确认未修**（本轮，含可执行证据） |
| 2 ZOH 重复计拍 / `log(abs(p))` | 已复核 → 前项不成立、后项属实并已修（§24） |
| 4 PE 激励门禁恒真 | 已复核 → 误诊（§23） |
| 3 IV 病态降级 | 未复核 |
| 5 Ljung-Box 自由度与 R² 免检 | 未复核 |
| 6 `operators/tuning.py` 不读 mode | 与 G24 同一问题（待规格裁决） |
...
**S3 累计：6 项已修属实 + 2 项确认待办（G24、G27#1）+ 4 项误诊。**

---

## 26. G27 复核（第 4 项）：Ljung-Box 自由度与 R² 免检——**两部分均成立**

### 26.1 自由度未扣除已估参数（**已修复**）

**事实依据**：对拟合了 p 个参数的模型，Ljung-Box 统计量 Q 渐近服从
**chi2(h − p)**（Box & Pierce 1970 / Ljung & Box 1978 的标准结论，h 为最大滞后阶）。
原实现 `p_value = 1 - chi2.cdf(Q, max_lag)`，**未扣除已估参数**。

后果是**反保守**方向的：自由度取大 → 临界值取大 → p 值偏大 →
**残差被判得比实际更白**，模型不充分时更容易通过"白噪声"检验。

实证（h=10、p=2）：

| Q | chi2(10) 的 p | chi2(8) 的 p |
|---|---|---|
| 15.5 | 0.1149 → **判白噪声** | 0.0501 → **恰在临界** |

**阈值附近结论相反**。更直接的证据：`select_order` 的签名里本就有 `n_params`
（且已传给 AIC/BIC），却**没有**传给 `ljung_box_test`——参数个数就在手边而未被使用。

**修复**：`ljung_box_test` 增加 `n_params` 形参（默认 0 保持旧行为），
自由度取 `max(1, h − p)`；两处调用点分别传 `na + nb`（pipeline）与
`n_params`（select_order）。

### 26.2 R² 免检（**确认存在，登记待裁决，未改**）
...
即 **R²_train 足够高时，白噪声检验被整段跳过**，`residual_white` 直接置真，
而该标志随后进入 `_assess_confidence`。**"R² 高"并不等于"残差白"**——
拟合优度高只说明模型解释了大部分方差，残差仍可能强自相关（系统性偏差）。

未改原因：这是**判断口径**而非数学错误——其隐含前提是"残差小到无所谓"，
是否接受取决于 `_R2_RESIDUAL_NEGLIGIBLE` 的取值与实际残差量级，
且改动面会波及所有高 R² 模型的可信度评估。按 G24/G27#4 同样处置：
登记证据与决策点，不擅改。

**决策点**：(a) 保持免检（承认"高 R² 下残差量级可忽略"）；或
(b) 取消免检，一律走白噪声检验（高 R² 模型的置信度可能普遍下降）；
或 (c) 折中——免检时把 `residual_white` 置为 `None`（未检验）而非 `True`，
使下游能区分"检验通过"与"未检验"。
...
| 环节 | 证据 |
|---|---|
| 修复前失败 | test_ljung_box_g27.py：**4 failed / 3 passed**（dof 口径、自由度下界、p 值单调性、select_order 传参四类全红） |
| 修复 | `order_selection.py` 自由度 `max(1, h−p)` + `n_params` 形参；两处调用点传参 |
| 行为测试 | 修复后 **7 passed**；含 dof=8 的精确对齐、缺省 chi2(h) 向后兼容、自由度下界为 1、p 值随 p 单调不增、阈值附近结论相反的量级固化、select_order 传参与 `residual_white` 一致性 |
| 集成证据 | 重新冻结后清单 diff **仅 order_selection.py 与 pipeline.py 2 条**（即本次改动文件）；全量门禁通过 |
| 残余风险 | ① 自由度修正使部分模型更难过白噪声检验 → **可信度可能下降、部分整定建议会更保守**，属行为变更需回归观察；② 与 G27#1（延误 BIC）同属"统计口径"类，建议一并观察；③ R² 免检部分未改（见 26.2 决策点） |

### 26.4 受保护清单解冻理由（逐文件）

- `app/services/tuning_identification/order_selection.py`：Ljung-Box 自由度按
  `h−p` 修正 + `n_params` 形参（G27#5）。
- `app/services/tuning_identification/pipeline.py`：调用点传入 `na+nb`（G27#5）。
  重新冻结后清单 diff **仅此 2 条**。
...
| 项 | 状态 |
|---|---|
| 1 BIC 跨样本量比较 | 已复核 → 成立，确认未修（§25） |
| 2 ZOH 重复计拍 / `log(abs(p))` | 已复核 → 前项不成立、后项已修（§24） |
| 4 PE 激励门禁恒真 | 已复核 → 误诊（§23） |
| 5 Ljung-Box 自由度 / R² 免检 | **已复核 → 两部分均成立；自由度已修，R² 免检登记待裁决**（本轮） |
| 3 IV 病态降级 | **未复核**（G27 最后一项） |
| 6 `operators/tuning.py` 不读 mode | 与 G24 同一问题（待裁决） |
| 7 `unitConversion` 缺转换 | 属 V62-P3-007 预留结构，非隐性缺陷 |

**S3 累计：7 项已修属实 + 2 项确认待办（G24、G27#1）+ 1 项部分待裁决（G27#5b）+ 4 项误诊。**

---

## 27. G27 复核（第 5 项，末项）：IV 病态降级——**成立，已可观测化**
...
1. **`np.linalg.solve` 仅在矩阵精确奇异时抛 `LinAlgError`**。**病态但非奇异**
   （真实场景：激励不足、工具变量与回归元近共线）时它会"成功"返回
   **数值上无意义**的 θ，**连告警都没有**——这是"病态降级"的核心。
...
按 G27#4 的教训（**阈值标定属决策**），本轮**不设阈值、不改判定行为**，
只让病态程度与求解路径可见——与 stiction 的 `range_source` 同一手法。

实测（600 点、一阶过程、d=3）：工具变量取外生 SP 时 `cond≈2547`，
取 u 自身时 `cond≈587`，两者均走 `solve`、均无异常——**正是原实现看不见的情形**。

（附带纠正一处我自己的错误假设：曾以为"工具变量退化为回归元会使条件数上升"，
实测相反（2547→587），故测试中不假设单调性，改为断言"字段随工具变量变化"。）

### 27.3 证据链

| 环节 | 证据 |
|---|---|
| 修复前失败 | test_iv_conditioning_g27.py：**4 failed / 0 passed**（两字段不存在） |
| 修复 | `iv.py` `IVResult` 增字段 + 两处求解点记录 cond 与路径 |
| 行为测试 | 修复后 **4 passed**；含良态记录、非奇异但求成功时仍记录 cond、字段随工具变量变化（证明实算）、旧构造点默认值兼容 |
| 集成证据 | 重新冻结后清单 diff **仅 iv.py 1 条**；全量门禁通过 |
| 残余风险 | ① **阈值与降权口径仍是决策**——`condition_number` 目前只被记录，未参与任何判定，故本项**不解决**"病态结果被当作正常结果使用"的问题，只使它**可见**；② 迭代式 `identify_clivc4`/`identify_iv4` 内部直接用 `lstsq`，未加记录（属伪逆路径，需否记录待定）；③ 建议与 G27#4（PE 条件数阈值）合并裁决，二者是同一类"数值条件 → 可信度"口径 |

### 27.4 受保护清单解冻理由（逐文件）

- `app/services/tuning_identification/iv.py`：`IVResult` 增条件数/求解路径字段
  （G27#3 可观测化）。重新冻结后清单 diff **仅该文件 1 条**。

### 27.5 G27 清单复核完成

| 项 | 状态 |
|---|---|
| 1 BIC 跨样本量比较 | 成立，**确认未修**（§25，含 xfail 证据） |
| 2 ZOH 重复计拍 / `log(abs(p))` | 前项**不成立**、后项**已修**（§24） |
| 3 IV 病态降级 | **成立，已可观测化；阈值待裁决**（本轮） |
| 4 PE 激励门禁恒真 | **误诊**（§23） |
| 5 Ljung-Box 自由度 / R² 免检 | **自由度已修**、R² 免检**待裁决**（§26） |
| 6 `operators/tuning.py` 不读 mode | 与 G24 同一问题（待裁决） |
| 7 `unitConversion` 缺转换 | 属 V62-P3-007 预留结构，非隐性缺陷 |

**G27 七项已全部复核完毕**（这是 S3 最后一项清单）。
结论分布：**3 项已修**（#2b/#5a/#3 可观测化）、**1 项确认未修**（#1）、
**2 项待裁决**（#5b、#3 阈值）、**1 项误诊**（#4）、**1 项非缺陷**（#7）、
**1 项与 G24 合并**（#6）。

**S3 累计：8 项已修属实 + 2 项确认待办（G24、G27#1）+ 2 项待裁决（G27#5b、#3阈值）+ 4 项误诊。**

---

## 28. G27#1 修复完成：延误搜索改用公共样本窗 BIC

§25 确认的缺陷（`_search_delay` 对每个候选 d 各用 `res.n_samples`，跨样本量比 BIC）
在本轮修复完毕。

### 28.1 修复要点

- 取对所有候选都成立的**公共窗** `t0 = max(na, d_max + nb)`；
- 用各候选的拟合系数在该窗上重算残差方差（新增
  `_residual_var_on_common_window`），以**同一 `n_common`** 计罚；
- 索引约定以 `cross_validate`（`order_selection.py:100-107`）给出的**权威形式**为准：
  `a[j]·y[t-1-j]`、`b[j]·u[t-d-j]`，**j 均从 0 起**。

**§25 失败教训的落地**：首版把 b 的索引写成 `u[t-d-j]` 而 j 从 1 起（**多减一拍**），
导致真值 d 落败；本轮先读准约定再改，一次通过。

### 28.2 修复后实测

| 观察 | 修复前 | 修复后 |
|---|---|---|
| 真值 d=8 是否选中 | 选中 | **选中**（BIC −10156.62，余量巨大） |
| "错误候选" BIC 跨度 | **68**（−6059…−5991，随 d **单调**漂移） | **2.2**（−5990.5…−5992.8，无单调性） |

即**跨样本量 artifact 已消除**，残余差异属 σ² 的真实噪声；延误搜索不再对大延迟
施加约两倍于合法罚项的额外保守度。
...
| 环节 | 证据 |
|---|---|
| 修复前失败 | §25 的 `xfail(strict=True)` 用例（BIC 随 d 大幅单调漂移） |
| 修复 | `pipeline.py` 公共窗 BIC + `_residual_var_on_common_window` |
| 行为测试 | 该用例**已由 xfail 转为通过**（标记已按约定删除，xfail 计数 12→11）；另一条"具明确滞后结构的数据仍被正确识别"持续通过 |
| 集成证据 | 重新冻结后清单 diff **仅 pipeline.py 1 条**；**全量门禁 0 失败**——延误选择结果虽有变化，但未破坏任何既有用例 |
| 残余风险 | ① 延误选择结果会变化（这正是修复目的），对**存量整定记录**而言新旧 θ 可能不同，建议在下一轮回归中抽查若干回路；② 与 G27#5a（Ljung-Box 自由度）同属"统计口径"类改动，建议合并观察；③ 公共窗使所有候选少用 d_max 个样本，n 略降——这是可比性的代价，属正确取舍 |

### 28.4 受保护清单解冻理由（逐文件）

- `app/services/tuning_identification/pipeline.py`：延误搜索改公共窗 BIC（G27#1）。
  重新冻结后清单 diff **仅该文件 1 条**。

### 28.5 S3 剩余

**10 项已修属实**（G20/G21/G23主路径/G25/G26/G27#2b/G27#5a/G27#3可观测化/G27#1/G27#4误诊勘正）。
剩余：**G24**（死参数，待裁决）、**G27#5b**（R² 免检，待裁决）、**G27#3 阈值**
（待裁决，建议与 G27#4 合并）、**G23 诊断链与整定链两步**（未统一）。

---

## 29. G23 续：诊断链 valid_rate 折入时间覆盖率（**已修复**）

§20 修复了 KPI 链主路径（门禁 + 落库），本轮补齐**诊断链**。

### 29.1 缺陷（修复前实测）

`DataQualityAssessor.assess()` 取裸 `compute_loop_valid_rate`，其注释自称
"可信度判定唯一输入"，但 **R14-2（2026-09-06）的 time_coverage 因子只落在
Pipeline 一处**——诊断链仍取裸值。

实测 R14-2 场景（120 点 / 30s 间隔 / 跨 1h / 契约 1s）：

| 链路 | valid_rate | 可信度 | 后果 |
|---|---|---|---|
| KPI 链（Pipeline） | 1.0 × 0.033 ≈ 0.033 | **E** | 指标 INCONCLUSIVE ✅ |
| 诊断链（assess） | **1.0（裸值）** | **A** | **通过诊断数据门禁**，算子在约 3% 覆盖的数据上产出"可信"结论 ❌ |

这正是 §20 所述"Phase 1 刚统一的口径被 R14-2 重新劈开"在本链路上的具体后果。

### 29.2 修复

`assess()` 折入 `compute_time_coverage`，`expected_interval_s` 用**契约**采样间隔
（`self.threshold.base_sampling_freq`）——`compute_time_coverage` 文档明确：
用实测中位间隔会把稀疏数据洗白成 coverage=100%。

### 29.3 证据链

| 环节 | 证据 |
|---|---|
| 修复前失败 | test_valid_rate_diagnosis_g23.py：**3 failed / 1 passed**（稀疏数据不得为 A 级、有效值 = 裸值×覆盖率、结构性守卫不足三类全红） |
| 修复 | `data_quality_assessor.py` `assess()` 折入覆盖率 |
| 行为测试 | 修复后 **4 passed**；含稀疏场景不得判 A、契约间隔下采满仍为 A（不误伤）、有效值量级 0.01~0.1、以及"结构性守卫为何漏掉它"的固化 |
| 集成证据 | 该文件**不在受保护清单内**；全量门禁通过 |
| 残余风险 | ① **整定链（`tuning.py:401`）仍取裸值**，三链统一尚差最后一步——该处需要契约采样间隔而当前只有块级实际 `sampling_freq`，用实际间隔会把稀疏数据洗白，故需先解决"整定链如何拿到契约间隔"；② 诊断链可信度将普遍下降（稀疏数据由 A 变 E），部分回路将不再产出诊断结论，属**有意的收紧**，建议回归观察；③ 与前两轮的 KPI 链修复合并看，三链口径正在收敛，但仍应把 R14-2 的口径写进规格，避免再被单边改动劈开 |

### 29.4 本轮过程事故与纠正（如实记录）

本轮出现一次**工具使用事故**并已完全清理：

1. 我用 `git stash push` + `pop` 做"修复前/后"对比。`pop` **弹错了条目**——
   弹到仓库中一个 2026-08-22 的**外部会话遗留 WIP**，在
   `endpoints/diagnosis_v2.py` 造成冲突并留下 `UU` 状态。
2. **已清理**：从 HEAD 恢复该文件（`git restore --source=HEAD --staged --worktree`），
   工作区与 HEAD 一致；**三个既有 stash 全部保留，未丢失他人 WIP**。
3. 随后连续三次"修复未落盘且无任何输出"。根因查明：我在 JS 模板字符串里写
   `'\n'`，被 **JS 提前解释为真实换行**，导致 Python 脚本语法错误——
   而我没有捕获 stderr，只看到"什么都没发生"。**教训：bash 探针必须捕获 stderr。**
...
## 31. G23 收口：整定链端到端数值断言（补齐 §30 的测试缺口）

§30 修复了整定链的时间覆盖率口径，但当轮只能做"来源断言"（`_fetch_preprocessed_signals`
依赖 DB + DataPlanner）。本轮用 `patch` 注入假 planner，把它升级为**数值断言**。

### 31.1 测试设计（一条断言同时守住两件事）

构造 PVOP_HF 块，把 `sampling_freq` 标为 **"30s"（实测采样率）**，而契约
（FLOW）为 **1s**；数据全 Good，故裸 valid_rate = 1.0。

- 若实现**未折入**覆盖率 → valid_rate ≈ 1.0；
- 若实现**误用实测间隔**算覆盖率 → coverage ≈ 1.0 → 仍 ≈ 1.0（洗白）；
- 只有**正确使用契约间隔** → 显著小于 1。

故"valid_rate 远小于 1"这一条断言同时守住"折入了覆盖率"与"用的是契约间隔"。

### 31.2 证据链

| 环节 | 证据 |
|---|---|
| 修复前失败（真实模拟） | 就地移除 `* time_coverage` 因子（模拟 §30 之前）→ **2 failed / 1 passed** |
| 修复 | §30 已提交（`6f348c43`）；本轮补端到端断言 |
| 行为测试 | 恢复后 **3 passed**；含有效值远低于裸值、量级区间、信号照常返回 |
| 集成证据 | 全量门禁通过；该文件不在受保护清单内 |
| 残余风险 | ① 假 planner 只覆盖 `request_bundles` 的返回形状，**不覆盖** DataPlanner/缓存/预处理链路本身——故本测试证明的是"该函数的口径正确"，不是"端到端链路正确"；② 精确量级（实测 ≈0.168）取决于 `compute_time_coverage` 对窗口/跨度的取值方式，故断言只锁"显著小于 1"这一可判定性质，未锁死具体数值 |

### 31.3 本轮的一次误判与纠正（如实记录）

第一次做"修复前"对比时用 `git checkout HEAD -- app/services/tuning.py`，
测试**修复前后都通过**，一度怀疑 §30 的修复无效。查明原因：**HEAD 已包含
§30 的修复**（上一轮已提交 `6f348c43`），故"恢复到 HEAD"恢复的是**已修版本**，
根本不是修复前版本。

改为**就地改写**（临时移除 `* time_coverage`）后，前后差异立刻显现
（2 failed → 3 passed）。

**纪律结论**：做"修复前/后"对比时，若修复已提交，`git checkout HEAD` 不再等于
"修复前"——必须明确地构造修复前状态（就地改写或取修复前的提交号），
不能想当然。
