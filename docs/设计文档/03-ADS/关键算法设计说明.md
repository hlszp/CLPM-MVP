...
|---|---|---|---|
| INCONCLUSIVE | ① 无数据（空 Bundle / 取数失败）；② 综合评分为 None——即有效自控率 R 缺失或可信度为 E 级（**R 是唯一会熔断整条快照的指标**） | NULL | 否（单独计数 inconclusiveLoops） |
| PARTIAL | 综合评分有值，但好值率/自控率/平稳率任一缺失 | 有值 | 否 |
| SUCCESS | 其余全部情形——**包括个别指标可信度为 E 级**：confidence_level 如实记为 E，但 status 仍为 SUCCESS | 有值 | 是 |

> 说明：原设计"任意指标 E 级 → 整条快照 INCONCLUSIVE 且不参与聚合"已废止。现行口径下 E 级只影响该指标自身的可信度标注与综合评分可信度（综合评分可信度取核心指标 + R 中的最低等级），快照是否 INCONCLUSIVE 仅由"无数据"或"R 为 E 级/缺失"决定。E 级 SUCCESS 快照仍参与装置级聚合与排行，页面以灰色标签提示"可信度 E"。
