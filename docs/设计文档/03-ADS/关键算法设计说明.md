...
**设计依据**: PRD (v4.0)、FDS (v4.0)、ADS (v4.0)、DDS (v4.0)、IDS (v4.0)、UI/UX (v4.0)、CLPM后端研发智能体系统提示词与开发指导文档
...
| stability_rate | BASE | PV, SP | BY_CONTROL_TYPE | pv_valid && sp_valid | oscillation_rate |
...
| composite_score | — | — | — | — | accuracy, fast, stability, effective_auto |
...
flowchart TD
    ...
    subgraph "核心质量指标计算"
        ...
        C3["稳定率 (S)<br/>calc_stability_rate"]
    end
...
| **输出** | | | |
| `stability_rate` | `Decimal(5,2)` | - | 稳定率（0~100%） |

#### 4.3.4 计算步骤

```
算法: calc_stability_rate
...
13. stability_rate = (1 / exp(normalized_std)) * osc_factor * 100
14. stability_rate = max(0, min(stability_rate, 100))
15. RETURN stability_rate
```
...
- $E_i$：第 $i$ 时刻控制偏差
- $|\bar{E}|$：控制偏差绝对值的平均值
- $|E|_{max}$：控制偏差绝对值的最大允许基准（默认量程的 5%）
- $r$：归一化偏差（0~1）
- $n$：有效数据点数
...
**指数衰减特性**：当 $|\bar{E}| = 0$ 时，$A = 100\%$；当 $|\bar{E}|$ 增大时，$A$ 呈指数衰减。因子 $(1 - 1/e^r)$ 使小偏差时扣分少，大偏差时扣分多，符合工业直觉。
...
6.  // 计算归一化偏差 r
7.  r = mean_abs_error / e_max
8.  // 指数衰减计算
9.  decay_factor = 1 - 1 / exp(r)
10. accuracy_rate = max(0, (1 - r * decay_factor)) * 100
11. accuracy_rate = min(accuracy_rate, 100)
12. RETURN accuracy_rate
```
...
|---|---|---|---|---|
| 稳定型 | 0.25 | 0.20 | 0.55 | 温度、压力控制 |
| 慢速型 | 0.30 | 0.15 | 0.55 | 缓慢调节回路 |
| 快速型 | 0.20 | 0.40 | 0.40 | 副回路、流量控制 |
| 逻辑型 | 0.00 | 0.20 | 0.80 | 防回流、防超温 |

> 权重存储于 `metric_config.weight` 字段，用户可在系统管理模块自助配置。
...
| `stability_rate` | `Float` | 是 | 稳定率（0~100） |
...
输入: accuracy_rate, fast_rate, stability_rate, effective_auto_rate, weights, status, valid_rate
...
11. // 计算加权平均
12. weighted_sum = accuracy_rate * a + fast_rate * f + stability_rate * s
...
| 指标名称 | 计算方法 | 说明 |
|---|---|---|
| 装置性能评分 | $\frac{\sum w_i \cdot Score_i}{\sum w_i}$ | 回路性能评分的加权平均 |
| 实时自控率 | $\frac{\text{当前时刻自控回路数}}{\text{总回路数}} \times 100\%$ | 实时状态统计 |
| 平均自控率 | $\frac{\sum w_i \cdot \text{有效自控率}_i}{\sum w_i}$ | 有效自控率的加权平均 |
| 平稳率 | $\frac{\sum w_i \cdot \text{稳定率}_i}{\sum w_i}$ | 稳定率的加权平均 |
...
6.  // 装置级 KPI 聚合
7.  FOR each kpi in [good_value_rate, effective_auto_rate, accuracy_rate, fast_rate, stability_rate, oscillation_rate, saturation_rate]:
...
|---|---|---|---|
| §6.3 单回路评估 | 通过有效自控率、平稳率、准确率、快速率评估 | §4.1~§4.11 实现 3+1 结构（准确率/快速率/稳定率+有效自控率）+ 8 辅助指标 | ✅ 合规（扩展） |
...
| 附录 B.5 平稳率 | $S = [1/\sigma \times (1 - Osc)] \times 100\%$ | §4.8 稳定率，指数型公式 $1/e^{\sigma/0.05U} \times (1-Osc)$ | ✅ 合规 |
...
| 装置平稳率 ≥ 95% | §4.11 装置级聚合 + 阈值告警（稳定率指标） | ✅ |
| 回路自控率 ≥ 95% | §4.6 有效自控率 + 阈值告警 | ✅ |
| 回路平稳率 ≥ 95% | §4.8 稳定率 + 阈值告警 | ✅ |
