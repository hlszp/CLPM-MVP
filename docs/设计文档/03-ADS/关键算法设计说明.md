...
**当前版本**: v1.0
**发布日期**: 2026-06-21
**文档性质**: ADS（系统架构设计说明书）v3.0 的正式补充文件
**设计依据**: PRD (v3.0)、FDS (v3.0)、ADS (v3.0)、DDS (v3.0)、IDS (v3.1)、UI/UX (v4.0)
...
| 版本 | 日期 | 变更说明 | 作者 |
|---|---|---|---|
| v1.0 | 2026-06-21 | 初始版本：①定义 6 大性能评估指标算法（好值率/自控率/平稳率/准确率/振荡率/饱和率）+ 综合评分 + 装置级聚合；②定义 8 类回路诊断算法（振荡检测/粘滞检测/参数过激/参数过保守/外扰频繁/PV质量码规则/诊断置信度融合/专家规则矩阵）；③定义 7 类 PID 整定算法（FOPDT/SOPDT 模型辨识 + IMC/Lambda/Z-N/Cohen-Coon/SIMC 整定 + 闭环仿真）；④对齐 GB/T 44693.2-2024 国标全部指标公式；⑤定义算法输入输出规范与数据类型；⑥时间/空间复杂度分析；⑦国标合规性矩阵；⑧算法验证方法 | 算法设计团队 |
...
| 编号 | 不一致点 | 本文档统一方案 |
|---|---|---|
| C1 | 6 大 KPI 列表在 PRD §5.1、FDS §5.3.1、工作台卡片、Mock kpi.ts 四处不一致 | 统一为：好值率、自控率、平稳率、准确率、振荡率、饱和率（对齐国标附录 B/F） |
...
**性能评估类（9 项）**：准确率公式、振荡率公式、饱和率公式、平稳率死区配置、IAE/ISE 计算、综合评分归一化、反向指标处理、INCONCLUSIVE 聚合、装置级 roll-up
...
本系统 6 大核心 KPI 严格对齐 GB/T 44693.2-2024 附录 B（性能评估指标）与附录 F（故障诊断指标）：

| 序号 | KPI 名称 | metric_code | 国标对应 | 算法类型 | 时间复杂度 |
|---|---|---|---|---|---|
| 1 | 好值率 | `GOOD_VALUE_RATE` | 附录 F.6 (Qu) | 质量码统计 | O(N) |
| 2 | 自控率 | `AUTO_MODE_RATE` | 附录 B.1 (Auto) | 模式统计 | O(N) |
| 3 | 平稳率 | `STEADY_RATE` | 附录 B.5 (S) | 偏差标准差 | O(N) |
| 4 | 准确率 | `ACCURACY_RATE` | 附录 B.3 (A) | 偏差绝对均值 | O(N) |
| 5 | 振荡率 | `OSCILLATION_RATE` | 附录 F.1 (Osc) | IAE 零交叉 | O(N) |
| 6 | 饱和率 | `SATURATION_RATE` | 附录 F.3 (Sa) | 输出限位统计 | O(N) |

> **注**：国标附录 B 还定义了"有效自控率（AR/R）"和"快速率（F）"，本系统将有效自控率作为自控率的子指标（区分"投自动"与"投自动且有效"），快速率作为 Phase 2 整定模块的输入（需稳态时间计算），不纳入 6 大 KPI 主表。
...
### 4.2 自控率（Auto Mode Rate）

#### 4.2.1 算法原理

统计评估时段内控制器处于自动（AUTO）或串级（CAS）模式的时长占比。对齐国标附录 B.1。

#### 4.2.2 数学模型

$$\eta_{auto} = \frac{T_{auto}}{T_{total}} \times 100\%$$

其中：
- $T_{auto}$：评估时段内 MODE 为 AUTO 或 CAS 的累计时长（秒）
- $T_{total}$：评估时段总时长（秒）
...
| `timestamps` | `Array<DateTime>` | 是 | 时间戳数组 |
| **输出** | | | |
| `auto_mode_rate` | `Decimal(5,2)` | - | 自控率（0~100%） |

#### 4.2.4 计算步骤

```
算法: calc_auto_mode_rate
输入: mode_values[], timestamps[]
输出: auto_mode_rate

1.  total_duration = timestamps[-1] - timestamps[0]
2.  IF total_duration == 0 THEN RETURN 0
3.  auto_duration = 0
4.  FOR i = 0 TO length(mode_values) - 2:
5.      segment_duration = timestamps[i+1] - timestamps[i]
6.      IF mode_values[i] IN {Auto, Cascade, Remote}:
7.          auto_duration += segment_duration
8.  auto_mode_rate = (auto_duration / total_duration) * 100
9.  RETURN auto_mode_rate
```
...
采用控制偏差标准差衡量 PV 波动平稳程度。对齐国标附录 B.5。

#### 4.3.2 数学模型

$$\eta_{steady} = \left[\frac{1}{\sigma} \times (1 - \eta_{osc})\right] \times 100\%$$
...
**归一化处理**：$\sigma$ 需除以量程范围 $PV_{range} = PV_{max} - PV_{min}$ 进行归一化，使平稳率无量纲且可跨回路比较：

$$\sigma_{norm} = \frac{\sigma}{PV_{range}}$$

最终公式：

$$\eta_{steady} = \max\left(0, \frac{1 - \eta_{osc} - k \cdot \sigma_{norm}}{1 - \eta_{osc}}\right) \times 100\%$$

其中 $k$ 为灵敏度系数（默认 $k = 10$，可配置），控制标准差对平稳率的影响程度。
...
| `sensitivity_k` | `Float` | 否 | 灵敏度系数（默认 10） |
| **输出** | | | |
| `steady_rate` | `Decimal(5,2)` | - | 平稳率（0~100%） |

#### 4.3.4 计算步骤

```
算法: calc_steady_rate
输入: pv_values[], sp_values[], pv_range, oscillation_rate, sensitivity_k=10
输出: steady_rate
...
6.  // 计算标准差
7.  std_error = sqrt(sum((errors[i] - mean_error)^2 for i) / n)
8.  // 归一化
9.  std_norm = std_error / pv_range
10. // 计算平稳率
11. osc_factor = 1 - oscillation_rate / 100
12. IF osc_factor <= 0 THEN RETURN 0
13. steady_rate = max(0, (osc_factor - sensitivity_k * std_norm) / osc_factor) * 100
14. steady_rate = min(steady_rate, 100)
...
$$\eta_{accuracy} = \left(1 - \frac{|\bar{E}|}{|E|_{max}}\right) \times 100\%$$
...
- $E_i$：第 $i$ 时刻控制偏差
- $|\bar{E}|$：控制偏差绝对值的平均值
- $|E|_{max}$：控制偏差绝对值的最大允许基准
- $n$：有效数据点数

**$|E|_{max}$ 取值规则**（可配置）：
- 默认：$|E|_{max} = PV_{range} \times 0.05$（量程的 5%）
- 回路级覆盖：`loop_ledger` 表可配置回路专属阈值
- 若 $|\bar{E}| \geq |E|_{max}$，则 $\eta_{accuracy} = 0$
...
5.  IF e_max == 0 THEN RETURN 0
6.  accuracy_rate = max(0, (1 - mean_abs_error / e_max)) * 100
7.  accuracy_rate = min(accuracy_rate, 100)
8.  RETURN accuracy_rate
```
...
基于 6 大 KPI 的加权综合评分。对齐国标附录 B.6 性能评分公式，扩展为 6 指标加权模型。
...
$$Score = \left(\sum_{i=1}^{6} w_i \cdot \eta_i^{norm}\right) \times R_{auto}$$

其中：
- $w_i$：第 $i$ 个 KPI 的权重（$\sum w_i = 1.0$）
- $\eta_i^{norm}$：第 $i$ 个 KPI 的归一化值（0~1）
- $R_{auto}$：有效自控率系数（0~1），作为整体折扣因子

**归一化规则**：

| KPI | 方向 | 归一化公式 | 说明 |
|---|---|---|---|
| 好值率 | 正向 | $\eta^{norm} = \eta / 100$ | 直接归一化 |
| 自控率 | 正向 | $\eta^{norm} = \eta / 100$ | 直接归一化 |
| 平稳率 | 正向 | $\eta^{norm} = \eta / 100$ | 直接归一化 |
| 准确率 | 正向 | $\eta^{norm} = \eta / 100$ | 直接归一化 |
| 振荡率 | **反向** | $\eta^{norm} = 1 - \eta / 100$ | 反向指标，振荡率越低分越高 |
| 饱和率 | **反向** | $\eta^{norm} = 1 - \eta / 100$ | 反向指标，饱和率越低分越高 |

**INCONCLUSIVE 处理**：
- 若好值率 < 20%，整条快照标记为 `INCONCLUSIVE`，评分留空（不以 0 分掩盖）
- 若某 KPI 因数据不足无法计算，该 KPI 权重按比例分配给其他 KPI

**有效自控率系数 $R_{auto}$**：

$$R_{auto} = \frac{T_{auto\_effective}}{T_{total}}$$

其中 $T_{auto\_effective}$ 为自控且输出不饱和且控制有效的时长（对齐国标附录 B.2 有效自控率）。

#### 4.7.3 默认权重配置

对齐国标附录 C 权重系数表，扩展为 6 指标：

| 控制类型 | 好值率 | 自控率 | 平稳率 | 准确率 | 振荡率 | 饱和率 | 适用场景 |
|---|---|---|---|---|---|---|---|
| 稳定型 | 0.10 | 0.10 | 0.30 | 0.15 | 0.20 | 0.15 | 温度、压力控制 |
| 慢速型 | 0.10 | 0.10 | 0.35 | 0.20 | 0.15 | 0.10 | 缓慢调节回路 |
| 快速型 | 0.10 | 0.15 | 0.20 | 0.10 | 0.25 | 0.20 | 副回路、流量控制 |
| 逻辑型 | 0.10 | 0.10 | 0.35 | 0.00 | 0.20 | 0.25 | 防回流、防超温 |

> 权重存储于 `metric_config.weight` 字段，用户可在系统管理模块自助配置，权重总和必须等于 1.0（100%）。
...
|---|---|---|---|
| **输入** | | | |
| `kpi_values` | `Object` | 是 | 6 大 KPI 值 `{good_value_rate, auto_mode_rate, steady_rate, accuracy_rate, oscillation_rate, saturation_rate}` |
| `weights` | `Object` | 是 | 6 大 KPI 权重 |
| `effective_auto_rate` | `Float` | 是 | 有效自控率（0~1） |
...
输入: kpi_values, weights, effective_auto_rate, status
输出: score

1.  IF status == INCONCLUSIVE THEN RETURN NULL
2.  // 归一化
3.  norm_values = {
4.      good_value_rate: kpi_values.good_value_rate / 100,
5.      auto_mode_rate: kpi_values.auto_mode_rate / 100,
6.      steady_rate: kpi_values.steady_rate / 100,
7.      accuracy_rate: kpi_values.accuracy_rate / 100,
8.      oscillation_rate: 1 - kpi_values.oscillation_rate / 100,  // 反向
9.      saturation_rate: 1 - kpi_values.saturation_rate / 100     // 反向
10. }
11. // 处理缺失 KPI（权重重分配）
12. active_weights = redistribute_weights(weights, kpi_values)
13. // 加权求和
14. weighted_sum = sum(active_weights[k] * norm_values[k] for k in keys)
15. // 乘以有效自控率系数
16. score = weighted_sum * effective_auto_rate * 100
...
**回路级别权重**（存储于 `loop_ledger.score_weight`）：

| 级别 | 描述 | 权重 |
|---|---|---|
| 一级 | 对装置整体性能/安全/经济/环保具决定性影响 | 3 |
| 二级 | 对装置运行稳定性或主要设备安全有较大影响 | 2 |
| 三级 | 相对次要的辅助控制回路 | 1 |

**INCONCLUSIVE 回路处理**：不参与聚合计算，但在装置级报告中单独统计数量与占比。

**装置级 KPI 聚合**：装置级自控率/平稳率等 KPI 同样按级别权重加权平均。
...
6.  // 装置级 KPI 聚合
7.  FOR each kpi in [good_value_rate, auto_mode_rate, ...]:
...
|---|---|---|---|
| §6.3 单回路评估 | 通过有效自控率、平稳率、准确率、快速率评估 | §4.1~§4.7 实现 6 大 KPI + 综合评分 | ✅ 合规（扩展） |
| 附录 B.1 自控率 | $Auto = AutoTime / AllTime \times 100\%$ | §4.2 实现一致 | ✅ 合规 |
| 附录 B.2 有效自控率 | $AR = AutoRealTime / AllTime \times 100\%$ | §4.7 综合评分中 $R_{auto}$ | ✅ 合规 |
| 附录 B.3 准确率 | $A = (1 - |E|/|E|_{max}) \times 100\%$ | §4.4 实现一致 | ✅ 合规 |
| 附录 B.5 平稳率 | $S = [1/\sigma \times (1 - Osc)] \times 100\%$ | §4.3 实现一致（扩展归一化） | ✅ 合规 |
| 附录 B.6 性能评分 | $P = [(A \cdot a + F \cdot f + S \cdot s)/(a+f+s)] \times R$ | §4.7 扩展为 6 指标加权 | ✅ 合规（扩展） |
| 附录 C 权重系数 | 稳定型/慢速型/快速型/逻辑型 | §4.7.3 默认权重配置 | ✅ 合规 |
| 附录 D 性能定级 | 一级至五级 | §4.7 评分映射等级 | ✅ 合规 |
| 附录 E.2 回路级别权重 | 一级(3)/二级(2)/三级(1) | §4.8 装置级聚合 | ✅ 合规 |
| 附录 F.1 振荡率 | IAE 零交叉相似率法 | §4.5 实现一致 | ✅ 合规 |
| 附录 F.3 饱和率 | $Sa = AutoSaturateTime / AllTime \times 100\%$ | §4.6 实现一致 | ✅ 合规 |
| 附录 F.5 黏滞系数 | $St = max(Ef,Of)/max(Er,Or) \times 100\%$ | §5.2 Choudhury + Kano | ✅ 合规（增强） |
| 附录 F.6 好值率 | $Qu = AutoQualityTime / AllTime \times 100\%$ | §4.1 实现一致 | ✅ 合规 |
| 附录 F.11 输出值行程指数 | $Trip = \sum|OP_i - OP_{i-1}| / t$ | §5.4 OP 活跃度（整定模块） | ✅ 合规 |
...
| 标准要求 | 本系统实现 | 合规状态 |
|---|---|---|
| 装置自控率 ≥ 95% | §4.8 装置级聚合 + 阈值告警 | ✅ |
| 装置平稳率 ≥ 95% | §4.8 装置级聚合 + 阈值告警 | ✅ |
| 回路自控率 ≥ 95% | §4.2 + 阈值告警 | ✅ |
| 回路平稳率 ≥ 95% | §4.3 + 阈值告警 | ✅ |
...
| `sensitivity_k` | 10 | 平稳率 | ✅ |
| `e_max_ratio` | 0.05 | 准确率 | ✅ |
