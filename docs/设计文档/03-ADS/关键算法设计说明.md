...
| v2.1 | 2026-09-13 | 补录 §3.7.3 有效可信度口径（回路级 valid_rate × 时间覆盖率，期望点数按**契约采样间隔**计；三链一致性强制；落库 valid_rate 与 confidence_level 同源）。该口径原仅存在于代码（R14-2），未入规格，曾被单边改动三次劈开 | 算法设计团队 |
...
| 冻结值 | 按控制类型的窗口检测（见§3.4.4） | 标记但保留（MARK_ONLY） | 传感器冻结或阀门卡死；2026-07-28 修订：不再直接置 invalid（避免误伤控制良好的平稳回路），是否计仪表故障由仪表故障率指标按复合判据（持续≥5min 且 OP 有变化而 PV 不动）判定 |
...
| OP_HF | OP | **固定1s** | 行程指数 | OP必须高频，捕捉阀门微小动作 |
| PVOP_HF | PV, OP | **固定1s** | 粘滞系数 | PV-OP散点拟合需要高频数据 |
| MODE_HF | MODE, OP | **固定1s** | 自控率、有效自控率、饱和率 | MODE切换需要精确时间点；饱和率需同源 OP 判定限位 |
...
| saturation_rate | MODE_HF | MODE, OP | FIXED_1S | mode_valid && op_valid | — |
...
| E | 数据不满足要求 | valid_rate < 0.20 | 仅作参考，如实标注 E 级（不再熔断整条快照，见下方"快照状态判定"） |

> **漏洞修复**：好值率不直接参与综合评分公式，而是影响指标可信度。好值率低→valid_rate低→可信度降级。

#### 3.7.3 有效可信度（回路级 valid_rate × 时间覆盖率）

> **本节为 2026-09-13 补录**。该口径原仅存在于代码实现（R14-2），未写入本文档，
> 导致其被单边改动**三次劈开**（Phase 1 统一 → R14-2 只落一处 → 各链路分别补回）。
> 现固化为规格，后续任何改动须先修订本节。

**定义**：

```
回路级 valid_rate   = 核心 tag（pv/sp/op/mode）同时有效的点数 / point_count
时间覆盖率         = 去重后实际点数 / 窗口期望点数（期望按**契约采样间隔**计）
有效可信度         = 回路级 valid_rate × 时间覆盖率
可信度等级         = ConfidenceEvaluator.evaluate(有效可信度)
```

**为什么必须乘时间覆盖率**：**有效点比例 ≠ 时间覆盖率**。
典型反例：120 个 Good 点、30s 间隔、跨 1 小时（契约采样间隔 1s）——
有效点比例 100%，但实际只采到约 3.3% 的时间点。
若只看有效点比例，此类稀疏数据会获得 A 级可靠评价；乘覆盖率后应为 E 级。

**关键约束（易错点）**：期望点数必须按**契约采样间隔**（如 FLOW=1s）计算，
**不得**使用实际中位采样间隔——后者会把上述稀疏数据"洗白"为覆盖率 100%，
使本机制完全失效。

**三链一致性（强制）**：以下三条链路**必须使用同一公式、同一契约间隔**，
任一条单独取裸 valid_rate 都会造成同回路同窗口的可信度自相矛盾：

| 链路 | 入口 |
|---|---|
| KPI | `pipeline.py`（回路级可信度）、门禁、落库快照 |
| 诊断 | `DataQualityAssessor.assess()` → 诊断数据门禁 |
| 整定 | `_fetch_preprocessed_signals` → 整定数据可信度 |

**落库一致性**：`kpi_snapshot_hourly.valid_rate` 与 `confidence_level`
必须同源，即 `ConfidenceEvaluator.evaluate(该行 valid_rate)` 应等于该行
`confidence_level`；跨表/跨链路读取 `valid_rate` 的消费者无需重新推导。


**快照状态判定（2026-07-20 按当前代码实现修订，对齐 `kpi_calc.py:1042-1112`）**：

| 状态 | 触发条件 | score | 是否参与聚合 |
|---|---|---|---|
| INCONCLUSIVE | ① 无数据（空 Bundle / 取数失败）；② 综合评分为 None——即有效自控率 R 缺失/E 级，**或任一参与评分的核心指标（准确率/快速率/稳定率）缺失/INCONCLUSIVE/E 级**（2026-07-28 D2 口径修订） | NULL | 否（单独计数 inconclusiveLoops） |
| PARTIAL | 综合评分有值，但好值率/自控率/平稳率任一缺失 | 有值 | 否 |
| SUCCESS | 其余全部情形——**包括个别辅助指标可信度为 E 级**：confidence_level 如实记为 E，但 status 仍为 SUCCESS | 有值 | 是 |

> 说明：原设计"任意指标 E 级 → 整条快照 INCONCLUSIVE 且不参与聚合"已废止。E 级只影响该指标自身的可信度标注与综合评分可信度（综合评分可信度取核心指标 + R 中的最低等级）。E 级 SUCCESS 快照仍参与装置级聚合与排行，页面以灰色标签提示"可信度 E"。
>
> **2026-07-28 D2 修订（综合评分缺失指标口径）**：原实现核心指标缺失时"分子计 0、分母留权重"，一个算不出的指标即把评分隐性拉低；现改为参与评分（weight>0）的核心指标任一缺失/INCONCLUSIVE/E 级 → 综合评分整体 INCONCLUSIVE（score=None，快照随之 INCONCLUSIVE）；核心指标 D 级保留评分并在 details 标注 `low_confidence_inputs`；weight=0 的核心指标（如逻辑型回路 fast_rate 权重为 0）缺失不熔断。
...
统计自控模式下控制器输出 OP 处于限位的时长占评估时段总时长的比例（分子=自控饱和时长，分母=评估时段总时长含手动）。对齐国标附录 F.3。
...
其中：
$$T_{auto\_saturated} = \sum_{i} \Delta t_i \cdot \mathbb{1}(mode_i \in Auto \land (OP_i \leq OP_{low} + \epsilon \lor OP_i \geq OP_{high} - \epsilon))$$
$$T_{total} = \sum_{i} \Delta t_i$$

参数说明：
- $T_{auto\_saturated}$：自控模式下 OP 处于限位的累计时长（秒，对应国标 AutoSaturateTime）
- $T_{total}$：评估时段总时长（秒，含手动模式，对应国标 AllTime）
- $OP_{low}$：输出下限（默认 0）
- $OP_{high}$：输出上限（默认 100）
- $\epsilon$：饱和容差（默认 2%，可配置）
- $\mathbb{1}(\cdot)$：指示函数
- $Auto$：自控模式集合 {AUTO, CAS, REMOTE, APC}（StandardMode 1/2/3/4）

**分子仅统计自控模式下的饱和时长**（手动模式不计入分子，但计入分母 $T_{total}$）。
对齐国标 GB/T 44693.2-2024 附录 F.3：$Sa = AutoSaturateTime / AllTime$；
全程手动时 $Sa=0\%$（非 INCONCLUSIVE）。
...
1.  total_duration = 0          # 分母：评估时段总时长（AllTime，含手动）
2.  auto_duration = 0           # 自控时长（诊断用）
3.  saturated_high = 0          # 分子：自控模式下高限饱和时长
4.  saturated_low = 0           # 分子：自控模式下低限饱和时长
5.  FOR i = 0 TO length(timestamps) - 1:
6.      segment = Δt_i          # 零阶保持：末点沿用前段时长
7.      total_duration += segment
8.      IF mode_values[i] IN {Auto, Cascade, Remote, APC}:
9.          auto_duration += segment
10.         IF op_values[i] 可解析:
11.             IF op_values[i] >= op_high - epsilon:
12.                 saturated_high += segment
13.             ELIF op_values[i] <= op_low + epsilon:
14.                 saturated_low += segment
15. IF total_duration == 0 THEN RETURN INCONCLUSIVE
16. saturation_rate = (saturated_high + saturated_low) / total_duration * 100
17. saturation_type = determine_type(saturated_high, saturated_low)
18. RETURN (saturation_rate, saturation_type)
```

注：自控模式含 AUTO/CAS/REMOTE/APC（StandardMode 1/2/3/4）。全程手动时
...
>
> **R2 权重口径裁决（2026-07-22，对齐 FDS v6.0 §5.3.1.2/§5.3.7.1）**：`metric_config.weight`（权重配置管理页面，按 `metric_code` 单值存储）为**唯一用户入口**；上表 4 类控制类型模板（`loop_type_weight` 表）降级为**出厂默认 / 兜底回退**。运行时优先级链：`MetricConfig.weight`（3 核心指标权重全部有效→归一化使用）> `LoopTypeWeight`（按回路 `control_type` 取出厂模板）> `None`（不计算综合评分），对应 `kpi_calc._build_weights_map` 实现。`effective_auto_rate` 作为折扣因子 R 不参与权重分配。
...
> **修订注记（2026-07-28）**：$A_1^*$ 是一阶加纯滞后过程的平均驻留时间，即 $A_1^* = \tau + \theta$。原步骤 3 写作 $\tau = A_1^*$，与步骤 4 单独估计的 $\theta$ 组合后总滞后变为 $\tau + 2\theta$，滞后被双重计入。现修正为 $\tau = A_1^* - \theta$；当 $A_1^* \le \theta$（即 $\tau \le 0$）时判定辨识失败，不输出带病参数。代码实现见 `backend/app/services/tuning_algorithms.py::_fopdt_area_method`。
...
$$\Delta u(k) = K_p \left[\Delta e(k) + \frac{T_s}{T_i} e(k) - \frac{T_d}{T_s} \Delta^2 PV(k)\right]$$

其中 $e(k) = SP(k) - PV(k)$，$T_s$ 为采样周期。

> **2026-07-28 修订**：微分项由对误差微分（$\Delta^2 e$）改为对 PV 微分（$-\Delta^2 PV$，微分先行），消除 SP 阶跃时的微分冲击（derivative kick）；与代码实现一致。
