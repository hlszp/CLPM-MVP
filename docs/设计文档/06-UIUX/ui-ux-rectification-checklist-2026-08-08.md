...
### 4.1 IA 再收敛（2026-08-09，当前实施批次）

本批次是 Phase 2 出口后的 IA follow-up，依据实现契约 v2.9 与 UI/UX v6.3：

| 事项 | 当前决策 | 状态 |
|---|---|---|
| 一级导航 | 7 项收敛为 6 项：监控 / 评估 / 诊断 / 整定 / 配置 / 系统 | ✅ 已实施 |
| 回路工作台归属 | 移除独立“回路”一级菜单；运行态统一到 `/monitor/loop-workbench` | ✅ 已实施 |
| 监控与工作台关系 | 左侧承担跨回路扫视，右侧四区承担同一回路监控→评估→诊断→整定；旧高密度表隐藏保留 | ✅ 已实施 |
| 预警规则 | `/config/alert-rules`，仅 ADMIN；不再作为一级菜单 | ✅ 已实施 |
| 预警结果 | `/monitor/alerts` + 全局通知铃铛，作为运行结果处理 | ✅ 已实施 |
| 旧入口 | `/loop/workbench`、`/loop/detail/:id`、`/alert/events`、`/alert/rules` 保留兼容重定向 | ✅ 已实施 |
| 工作台可读性 | 概览自适应高度；列表补 PV/SP/OP/模式；空态补原因/范围/下一步；Emoji 替换为 Lucide；E 级可信度中性 | ✅ 已实施 |

该方案明确“合并导航和上下文，不合并所有视觉任务”：跨回路巡检与单回路深度处置属于不同认知任务，保留两层信息密度，避免工作台变成不可读的超级表格。后续“实时监控 + 关注队列 + 生命周期 + 实施验证 + 批量视图”已形成《监控—回路工作台闭环整改实施计划》及配套任务清单，状态为待开工，不计入本 Phase 已完成项：`monitor-workbench-rectification-plan-2026-08-09.md`、`monitor-workbench-rectification-checklist-2026-08-09.md`。
...
| 2026-08-09 | 后端宕死排查加固：echo 关停 + command_timeout=60 + 噪音日志钳制 | `2b9fb9d` | 现场取证：~1.1 万 TIME_WAIT 到 Redis 的连接风暴；echo 关停实测生效；pytest 4139 全绿 |
