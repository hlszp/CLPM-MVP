...
> 对象：`/monitor/loop-workbench`（组件仍为 views/loop/workbench.vue，旧 `/loop/workbench` 仅作兼容重定向）
...
> **v2.9 follow-up 状态（2026-08-09）**：本轮 IA 收敛已处理 W-1~W-7 的代码项（概览自适应、空态说明、整定前置提示、趋势空态、Emoji、算法/可信度映射、持平降噪）；W-8（信息提示 toast 语义）、W-9（趋势动作层级）、W-10（切换回路请求风暴）仍作为后续优化项，不影响本轮导航合并。
