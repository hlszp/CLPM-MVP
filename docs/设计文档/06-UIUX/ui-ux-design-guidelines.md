...
- **右侧表单**：选中指标时展示
  ...
  - **权重** [v5.3 修订]：仅核心质量指标可编辑，百分比数值输入，0-100%，带实时总和显示与校验；投用指标显示"折扣因子"标签，无权重输入；辅助诊断指标显示"不参与评分"，权重字段禁用
    - **注意**：核心指标权重按控制类型分 4 套模板，本页编辑的是当前控制类型下的权重值；如需管理权重模板，请前往 §6.3.6 权重配置管理页面
...
- **控制类型字段迁移** [v5.3 重要变更]：`control_type` 字段从 `metric_config` 迁移至 `loop_ledger`（对齐 DDS v4.1）。本页不再配置控制类型，改为在 §6.3.6 权重配置管理页面按控制类型分 4 套模板统一管理
