...
**文档状态**: 正式版
**当前版本**: v6.3（监控—回路工作台 IA 收敛版，继承 v6.2 色彩与工业组件规范）
**发布日期**: 2026-07-06
...
| v6.3 | 2026-08-09 | IA 再收敛：一级菜单收敛为监控/评估/诊断/整定/配置/系统 6 项；回路工作台归入监控并作为单回路监控→评估→诊断→整定闭环入口；预警事件并入监控、预警规则并入配置；跨回路扫视保留在工作台左侧高密度列表，旧实时表仅作隐藏批量视图；工作台概览自适应、补齐 PV/SP/OP/模式摘要、空态三要素、Lucide 功能图标与中性可信度表达。 | 系统设计团队（AI 代理执行，用户验收） |
...
### 1.5 当前 IA 与工作台闭环（v6.3）

实现契约 v2.9 对历史“6 模块 + 门户”或“7 模块”描述作当前覆盖。用户可见的一级模块为：**监控 / 评估 / 诊断 / 整定 / 配置 / 系统**。

监控与回路工作台采用“导航合并、任务分层”设计：

- `/monitor/loop-workbench` 是回路实体的唯一运行态入口。左侧回路列表负责跨回路扫视，显示位号、PV/SP/OP、模式、评分/较昨日趋势、可信度；右侧以同一 `loopId` 连续呈现概览、性能评估、诊断、整定四区。`/monitor` 按角色将 ADMIN/IC/PE/SPONSOR 导向系统概览、EXPERT 导向该工作台，避免 Sponsor 或专家落入无权页面。
- 跨回路扫视与单回路深度处置不压缩成一张超宽表。列表只保留选择和优先级所需信息，完整诊断证据、评估趋势和整定动作只在右侧工作台展开。
- `/loop/monitor` 作为隐藏的高密度实时表保留，用于大批量巡检、导出和旧书签兼容；不在一级菜单中制造第二个“监控”入口。
- 预警事件是运行结果，归 `/monitor/alerts`，并通过全局通知铃铛提供跨页面入口；预警规则是结构性配置，归 `/config/alert-rules`，仅管理员可维护。

工作台四区的首屏顺序必须遵循“先状态、再证据、后动作”：概览使用自适应高度，不以固定 10% 裁切内容；评估、诊断、整定区都必须表达“当前是否有数据、缺失原因、数据范围和下一步动作”。`INCONCLUSIVE`、E 级可信度、无数据与零值使用中性表达，不得以红色制造错误暗示；功能图标使用 Lucide 等语义化图标，不使用 Emoji。
